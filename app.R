...
# Read CSV directly
df <- read.csv(text = rawToChar(res$content$download_url), stringsAsFactors = FALSE)
colnames(df) <- tolower(trimws(colnames(df)))
if (!all(c("name", "frequency", "district") %in% colnames(df))) {
    stop("CSV must contain 'name', 'frequency', and 'district' columns.",download_url,rawToChar(res$content$download_url),colnames(df))
  }
